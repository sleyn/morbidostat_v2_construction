...
void TEMP_Init(void)
{
  BOOL b;
  UINT16 u;
  if (NVS_ReadTag(NVTAG_TempOn, &b, sizeof(b)) != sizeof(b)) b = FALSE;
  TEMP_SetTempOn(b);
  if (NVS_ReadTag(NVTAG_Temp, &u, sizeof(u)) != sizeof(u)) u = 370;
  TEMP_SetTemp(u);
  TEMP_ResetTempMin();
  TEMP_ResetTempMax();
  HWC_SetHeater(OFF);
...
void TEMP_ToggleTempOn(void)
{
  ...
  NVS_WriteTag(NVTAG_TempOn, &TempOn, sizeof(TempOn));
}
