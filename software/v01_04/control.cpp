...
static UINT32 InitConcentration;
static UINT32 InConcentrationL;
static UINT32 InConcentrationH;
static UINT32 Multiplier; // in 0.1 units
static UINT32 Vtube;
static int DilXcount;
static int GrowthCorrectionEod; // in 0.001
static int GrowthCorrectionEur; // in 0.001
static int GrowthCorrectionEuf; // in 0.001
static BOOL DilOn;
static BOOL OdOn;
...
static UINT32 lastOD[NUM_TUBES];
static int DilTube = INVALID_TUBE;
static int cleanCycle;
...
void CTL_Init(void)
{
  ...
  {
    CTL_SetThresholdLow(i, 200);
    CTL_SetThresholdDrug(i, 400);
    CTL_SetThresholdUpper(i, 800);
    CTL_SetDilPeriodBelowLT(i, 60);
    CTL_SetDilPeriodAboveLT(i, 15);
    CTL_SetAddingVolume(i, 20);
    CTL_SetLaserStrength(i, 4);
    CTL_SetTubeOn(i, TRUE);
  ...
  CTL_SetDilPeriodUpper(120);
  CTL_SetConcentration(100);
  CTL_SetInConcentrationL(0);
  CTL_SetInConcentrationH(1000);
  CTL_SetMultiplier(15);
  CTL_SetDilXcount(3);
  CTL_SetGrowthCorrectionEod(0);
  CTL_SetGrowthCorrectionEur(0);
  CTL_SetGrowthCorrectionEuf(0);
  CTL_SetMode(eMode_Off);
  CTL_SetAirDelay(5000);
  CTL_SetAirOn(FALSE);
  CTL_SetOdTimer(7000);
  CTL_SetVtube(100);
...
void ctl_OD(int tube, UINT32 od)
{
  ...
  static UINT32 OdAvg[NUM_TUBES], LastOdAvg[NUM_TUBES];
  static int OdCount[NUM_TUBES], DilCount[NUM_TUBES];
  static BOOL DilDrug[NUM_TUBES];
  static UINT32 DrugZoneCount[NUM_TUBES];
  ...
  switch (TubeState[tube])
  {
    ...
    case TUBST_Upper:
      ...
      {
        Concentration[tube] = (Concentration[tube] * Multiplier + 5) / 10;
      ...
      {
        ctl_Dilution(tube, AddingVolume[tube], InConcentrationL);
        UART_PrintfMsg("Tube %d: Dilution Zone 4 Upper, P1 only", tube + 1);
      }
      lastMode[tube] = 0;
      break;

    case TUBST_Drug:
      if (oldState != TUBST_Drug) DrugZoneCount[tube] = 0;
      OdAvg[tube] += od;
      OdCount[tube]++;
      if (gTimerTicks >= LastDilusionTicks[tube] + DilPeriodAboveLT[tube])
      {
        OdAvg[tube] /= OdCount[tube];
        rate = OdAvg[tube] - LastOdAvg[tube];

        if (++DrugZoneCount[tube] < 4 ||
            rate > LastRate[tube] + (rate > 0 ? GrowthCorrectionEur : GrowthCorrectionEuf))
        { // Mode 1
        ...
        else if (OdAvg[tube] > LastOdAvg[tube] + GrowthCorrectionEod)
        { // Mode 2
          if (DilDrug[tube])
          {
            if (DilXcount > 1)
            {
              DilCount[tube] = DilXcount - 1;
              DilDrug[tube] = FALSE;
            }
            ctl_Dilution(tube, AddingVolume[tube], Concentration[tube]);
...
void CTL_DilTask(UINT32 msg)
{
    ...
    case DILMSG_TIMER: 
      switch (DilState)
      {
        ...
        case DILST_AirOn:
          nextTube = TRUE;
          if (cleaning)
            UART_PrintfMsg("Finished cleaning Tube %d Cycle %d", DilTube + 1, cleanCycle);
          break;
      }
      break;
  ...
  if (nextTube)
  {
    for (i = 0; i < NUM_TUBES; i++)
    {
      ...
      {
        UART_PrintfPumpMsg("Tube%d\t%ld\t%ld\t%d\t%d\t%ld.%03ld", DilTube + 1,
                           DilParams[DilTube].timeL, DilParams[DilTube].timeH,
                           TubeState[DilTube] + 1, lastMode[DilTube],
                           lastOD[DilTube] / 1000, lastOD[DilTube] % 1000);
      }
...
void CTL_SetMultiplier(UINT32 mult)
{
  Multiplier = mult;
  GUI_Multiplier(mult);
  UART_PrintfMsg("Set Multiplier %ld.%ld", mult / 10, mult % 10);
}
...
void CTL_SetDilXcount(int x)
{
  DilXcount = x;
  GUI_DilXcount(x);
  UART_PrintfMsg("Set X %d", x);
}

void CTL_SetGrowthCorrectionEod(int e)
{
  GrowthCorrectionEod = e;
  GUI_GrowthCorrectionEod(e);
  if (e < 0) e = -e;
  UART_PrintfMsg("Set Eod %c%d.%03d", (GrowthCorrectionEod < 0) ? '-' : ' ', e / 1000, e % 1000);
}

void CTL_SetGrowthCorrectionEur(int e)
{
  GrowthCorrectionEur = e;
  GUI_GrowthCorrectionEur(e);
  if (e < 0) e = -e;
  UART_PrintfMsg("Set Eur %c%d.%03d", (GrowthCorrectionEur < 0) ? '-' : ' ', e / 1000, e % 1000);
}

void CTL_SetGrowthCorrectionEuf(int e)
{
  GrowthCorrectionEuf = e;
  GUI_GrowthCorrectionEuf(e);
  if (e < 0) e = -e;
  UART_PrintfMsg("Set Euf %c%d.%03d", (GrowthCorrectionEuf < 0) ? '-' : ' ', e / 1000, e % 1000);
}

void CTL_SetMode(int mode)
{
  static int morbMode = eMode_OdReadings;
  ...
  if (mode != morbMode) 
  {
    ...
    switch (mode)
    {
      ...
      case eMode_Morbidostat:
        OdOn = TRUE;
        DilOn = TRUE;
        cleaning = FALSE;
        UART_PrintfMsg("Set Mode Morbidostat");
        CTL_PrintAllParams();
        break;
    ...
    GUI_Mode(mode);
    if (cleaning) OS_Message(TASK_DIL, DILMSG_NEW);
  }
}
...
void CTL_SetVtube(UINT32 mL)
{
  Vtube = mL;
  GUI_Vtube(mL);
  UART_PrintfMsg("Set Tube Volume %ld", mL);
}
...
void CTL_SetConcentration(UINT32 c)
{
  ...
  InitConcentration = c;
  for (i = 0; i < NUM_TUBES; i++) Concentration[i] = InitConcentration;
  GUI_Concentration(c);
  UART_PrintfMsg("Set Concentration %ld", c);
}
...
{
  int i;
  int eod = GrowthCorrectionEod, eur = GrowthCorrectionEur, euf = GrowthCorrectionEuf;
  ...
  UART_PrintfMsg("============================================================");
  UART_PrintfMsg("tube    LT     DT     UT     TBL    TAL    dV    C  Zone");
  UART_PrintfMsg("                            (min)  (min)  (mL)");
  UART_PrintfMsg("============================================================");

  for (i = 0; i < NUM_TUBES; i++)
  {
    UART_PrintfMsg("%2d   %2ld.%03ld %2ld.%03ld %2ld.%03ld  %5ld  %5ld %4ld.%1ld %4ld %d-%s",
                   i + 1,
                   ...
                   Concentration[i],
                   TubeState[i] + 1, zoneName[TubeState[i]]);
  }
  UART_PrintfMsg("============================================================");
  UART_PrintfMsg(" C = %ld, C1 = %ld, C2 = %ld,   M = %ld.%0ld, X = %d, V = %ld",
                 InitConcentration, InConcentrationL, InConcentrationH,
                 Multiplier / 10, Multiplier % 10, DilXcount, Vtube);
  UART_PrintfMsg(" Eod = %c%d.%03d, Eur = %c%d.%03d, Euf = %c%d.%03d, Tupper = %ld",
                 GrowthCorrectionEod < 0 ? '-' : ' ', eod / 1000, eod % 1000,
                 GrowthCorrectionEur < 0 ? '-' : ' ', eur / 1000, eur % 1000,
                 GrowthCorrectionEuf < 0 ? '-' : ' ', euf / 1000, euf % 1000,
                 DilPeriodUpper / SEC_TICKS);
